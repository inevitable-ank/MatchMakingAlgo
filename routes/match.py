...
def get_next_profile(viewer_id: str):
    ...
    print(f"[API] GET /api/next-profile/{viewer_id} at {time.time()}")
    # Ensure viewer exists
    viewer = USER_STORE.get(viewer_id)
    if viewer is None:
        print(f"[API] Viewer {viewer_id} not found")
        raise HTTPException(status_code=404, detail="Viewer not found")
    # Recommend profile
    profile = recommend_next_profile(viewer_id)
    if profile is None:
        print(f"[API] No more profiles for viewer {viewer_id}")
        raise HTTPException(status_code=204, detail="No more profiles available")
    print(f"[API] Recommended profile {profile.id} for viewer {viewer_id}")
    return profile

@router.post("/feedback")
def post_feedback(feedback: Feedback):
    """Record user feedback (like/dislike) for a profile"""
    print(f"[API] POST /api/feedback for viewer {feedback.viewer_id} profile {feedback.profile_id} liked={feedback.liked} at {time.time()}")
    # Validate viewer and profile
    if feedback.viewer_id not in USER_STORE:
        print(f"[API] Viewer {feedback.viewer_id} not found")
        raise HTTPException(status_code=404, detail="Viewer not found")
    if str(feedback.profile_id) not in USER_STORE:
        print(f"[API] Profile {feedback.profile_id} not found")
        raise HTTPException(status_code=404, detail="Profile not found")
    # Record feedback
    record_feedback(feedback.viewer_id, feedback.profile_id, feedback.liked)
    print(f"[API] Feedback recorded for viewer {feedback.viewer_id} profile {feedback.profile_id}")
